...
namespace back {
	...
	struct sub_sm<sm_impl<sm_policy<T>>> {
		...
		template <class... TPolicies>
		static const sm_impl<sm_policy<T, TPolicies...>>& cget(const aux::pool_type<sm_impl<sm_policy<T, TPolicies...>>>*
		object) {
...
namespace back {
	...
	template <class TSM_, class T, class... Ts>
	struct transitions_sub<sm<TSM_>, T, Ts...> {
		template <class TEvent, class SM, class TDeps, class TSubs>
		static bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state) {
		...
		static bool execute(const anonymous& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state) {
			if (sub_sm<sm_impl<TSM_>>::cget(&subs).is_terminated()) {
				const auto handled = sub_sm<sm_impl<TSM_>>::get(&subs).process_event(event, deps, subs);
		...
		static bool execute_impl(const back::on_entry<_, TEvent>& event, SM& sm, TDeps& deps, TSubs& subs,
			...
			sub_sm<sm_impl<TSM_>>::get(&subs).process_event(event, deps, subs);
			return true;
		}
		template <class _, class TEvent, class SM, class TDeps, class TSubs>
	...
	template <class TSM_>
	struct transitions_sub<sm<TSM_>> {
		template <class TEvent, class SM, class TDeps, class TSubs>
		static bool execute(const TEvent& event, SM&, TDeps& deps, TSubs& subs, typename SM::state_t&) {
			return sub_sm<sm_impl<TSM_>>::get(&subs).process_event(event, deps, subs);
		}
		template <class, class SM, class TDeps, class TSubs>
...
namespace back {
	...
	template <class T, class TMappings>
	struct get_event_mapping_impl_helper<exception<T>, TMappings> : decltype(get_event_mapping_impl<exception<T>>((
	            TMappings*)0)) {
	};
...
namespace back {
	namespace policies {
		...
		struct fold_expr {
			...
			static bool dispatch_impl(sm_impl& self, State& current_state, aux::index_sequence<Ns...>, const TEvent& event,
			                          TDeps& deps,
...
namespace back {
	template <class TSM_>
	struct sm_impl : aux::conditional_t<aux::is_empty<typename TSM_::sm>::value, aux::none_type, typename TSM_::sm> {
		using sm_t = typename TSM_::sm;
		using thread_safety_t = typename TSM_::thread_safety_policy::type;
		template <class T>
		using defer_queue_t = typename TSM_::defer_queue_policy::template rebind<T>;
		using defer_flag_t = typename TSM_::defer_queue_policy::flag;
		template <class T>
		using process_queue_t = typename TSM_::process_queue_policy::template rebind<T>;
		using logger_t = typename TSM_::logger_policy::type;
		using dispatch_t = typename TSM_::dispatch_policy;
		...
		sm_impl(const TPool& p, aux::false_type) : sm_t{ aux::try_get<sm_t>(&p) }, transitions_{ (*this)() } {
			initialize(typename sm_impl<TSM_>::initial_states_t{});
		}
		template <class TPool>
	...
	class sm {
		...
		private:
			using sm_all_t =
			    aux::apply_t<get_non_empty_t, aux::join_t<aux::type_list<sm_t>, aux::apply_t<get_sm_t, state_machines>>>;
			using sub_sms_t =
			    aux::apply_t<aux::pool,
			    typename convert_to_sm<TSM_, aux::apply_t<aux::unique_t, aux::apply_t<get_sub_sms, states>>>::type>;
			using deps = aux::apply_t<merge_deps, transitions_t>;
			using deps_t =
			    aux::apply_t<aux::pool,
		...
		public:
			sm() : deps_{ aux::init{}, aux::pool<>{} }, sub_sms_{ aux::pool<>{} } { aux::get<sm_impl<TSM_>>(sub_sms_).start(deps_, sub_sms_); }
			...
			explicit sm(TDeps&& deps) : deps_ { aux::init{}, aux::pool<TDeps> {deps} }, sub_sms_{ aux::pool<TDeps>{deps} } {
				aux::get<sm_impl<TSM_>>(sub_sms_).start(deps_, sub_sms_);
			}
			template < class... TDeps, __BOOST_SML_REQUIRES((sizeof...(TDeps) > 1)&& aux::is_unique_t<TDeps...>::value) >
			explicit sm(TDeps&& ... deps) : deps_{ aux::init{}, aux::pool<TDeps...>{deps...} }, sub_sms_{ aux::pool<TDeps...>{deps...} } {
				aux::get<sm_impl<TSM_>>(sub_sms_).start(deps_, sub_sms_);
			}
			sm(aux::init, deps_t& deps) : deps_{ deps }, sub_sms_{ deps } { aux::get<sm_impl<TSM_>>(sub_sms_).start(deps_, sub_sms_); }
			sm(const sm&) = default;
			sm(sm&&) = default;
			...
			bool process_event(const TEvent& event) {
				return aux::get<sm_impl<TSM_>>(sub_sms_).process_event(event, deps_, sub_sms_);
			}
			template < class TEvent, __BOOST_SML_REQUIRES(!aux::is_base_of<TEvent, events_ids>::value) >
			bool process_event(const TEvent& event) {
				return aux::get<sm_impl<TSM_>>(sub_sms_).process_event(unexpected_event<_, TEvent> {event}, deps_, sub_sms_);
			}
			...
			void visit_current_states(const TVisitor& visitor) const {
				...
				using sm_impl_t = sm_impl<typename TSM_::template rebind<type>>;
			...
			bool is(const TState&) const {
				...
				return aux::get_id<state_t, typename TState::type>((states_ids_t*)0) == aux::cget<sm_impl_t>
				       (sub_sms_).current_state_[0];
			}
			...
			template <class T = aux::identity<sm_t>, class... TStates,
			          __BOOST_SML_REQUIRES(sizeof...(TStates) == sm_impl<typename TSM_::template rebind<typename T::type>>::regions)>
			...
			template < class T = aux::identity<sm_t>, class... TStates,
			           __BOOST_SML_REQUIRES(!aux::is_same<no_policy, typename TSM_::testing_policy>::value && aux::always<T>::value) >
			...
			template <class T>
			operator T& () {
				return aux::get<sm_impl<typename TSM_::template rebind<T>>>(sub_sms_);
			}
			template <class T>
			operator const T& () {
				return aux::cget<sm_impl<typename TSM_::template rebind<T>>>(sub_sms_);
			}
...
namespace front {
	...
	struct action_base {};
	template <class TRooTSM_, class... TSubSMs>
	TRooTSM_ get_root_sm_impl(aux::pool<TRooTSM_, TSubSMs...>*);
	...
	template <class T, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<T>&, const TEvent&, TSM_&, TDeps& deps) {
		return aux::get<T>(deps);
	}
	template <class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<TEvent>&, const TEvent& event, TSM_&, TDeps&) {
		return event;
	}
	template <class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<const TEvent&>&, const TEvent& event, TSM_&, TDeps&) {
		return event;
	}
	template <class T, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<const TEvent&>&, const back::unexpected_event<T, TEvent>& event, TSM_&, TDeps&) {
		return event.event_;
	}
	template <class T, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<const TEvent&>&, const back::on_entry<T, TEvent>& event, TSM_&, TDeps&) {
		return event.event_;
	}
	template <class T, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<const TEvent&>&, const back::on_exit<T, TEvent>& event, TSM_&, TDeps&) {
		return event.event_;
	}
	template <class T, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<const TEvent&>&, const back::exception<T, TEvent>& event, TSM_&, TDeps&) {
		return event.exception_;
	}
	template <class... TEvents, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<back::defer<TEvents...>>&, const TEvent, TSM_& sm, TDeps&) {
		return back::defer<TEvents...> {sm.defer_};
	}
	template <class... TEvents, class TEvent, class TSM_, class TDeps>
	decltype(auto) get_arg(const aux::type<back::process<TEvents...>>&, const TEvent, TSM_& sm, TDeps&) {
	...
	struct call<TEvent, aux::type_list<>, back::no_policy> {
		template <class T, class TSM_, class TDeps, class TSubs>
		static auto execute(T object, const TEvent&, TSM_&, TDeps&, TSubs&) {
			return object();
		}
	};
	...
	struct call<TEvent, aux::type_list<>, TLogger> {
		template <class T, class TSM_, class TDeps, class TSubs>
		static auto execute(T object, const TEvent& event, TSM_&, TDeps& deps, TSubs&) {
			using result_type = decltype(object());
			return execute_impl<typename TSM_::sm_t>(aux::type<result_type> {}, object, event, deps);
		}
		template <class TSM_, class T, class TDeps>
		static auto execute_impl(const aux::type<bool>&, T object, const TEvent& event, TDeps& deps) {
			const auto result = object();
			back::policies::log_guard<TSM_>(aux::type<TLogger> {}, deps, object, event, result);
			return result;
		}
		template <class TSM_, class T, class TDeps>
		static auto execute_impl(const aux::type<void>&, T object, const TEvent& event, TDeps& deps) {
			back::policies::log_action<TSM_>(aux::type<TLogger> {}, deps, object, event);
			object();
		}
	};
	template <class TEvent>
	struct call<TEvent, aux::type_list<TEvent>, back::no_policy> {
		template <class T, class TSM_, class TDeps, class TSubs>
		static auto execute(T object, const TEvent& event, TSM_&, TDeps&, TSubs&) {
			return object(event);
		}
	};
	template <class TEvent, class TLogger>
	...
	struct call<TEvent, aux::type_list<action_base>, back::no_policy> {
		template <class T, class TSM_, class TDeps, class TSubs>
		static auto execute(T object, const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
	...
	struct call<TEvent, aux::type_list<Ts...>, back::no_policy> {
		template <class T, class TSM_, class TDeps, class TSubs>
		static auto execute(T object, const TEvent& event, TSM_& sm, TDeps& deps, TSubs&) {
	...
	struct call<TEvent, aux::type_list<Ts...>, TLogger> {
		...
		static auto execute(T object, const TEvent& event, TSM_& sm, TDeps& deps, TSubs&) {
			...
			return execute_impl<typename TSM_::sm_t>(aux::type<result_type> {}, object, event, sm, deps);
		}
		template <class TSM_, class T, class SM, class TDeps>
	...
	class seq_ : operator_base {
		public:
			...
			template <class TEvent, class TSM_, class TDeps, class TSubs>
			void operator()(const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
				for_all(aux::make_index_sequence<sizeof...(Ts)> {}, event, sm, deps, subs);
			}

		private:
			template <int... Ns, class TEvent, class TSM_, class TDeps, class TSubs>
			void for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
				#if defined(__cpp_fold_expressions)
				(call<TEvent, args_t<Ts, TEvent>, typename TSM_::logger_t>::execute(aux::get_by_id<Ns>(&a), event, sm, deps, subs),
				 ...);
				#else
				(void)aux::swallow {
					0, (call<TEvent, args_t<Ts, TEvent>, typename TSM_::logger_t>::execute(aux::get_by_id<Ns>(&a), event, sm, deps, subs),
					    0)...
				};
				#endif
			}
	...
	class and_ : operator_base {
		public:
			...
			template <class TEvent, class TSM_, class TDeps, class TSubs>
			auto operator()(const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
				return for_all(aux::make_index_sequence<sizeof...(Ts)> {}, event, sm, deps, subs);
			}

		private:
			template <int... Ns, class TEvent, class TSM_, class TDeps, class TSubs>
			auto for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
				#if defined(__cpp_fold_expressions)
				return (call<TEvent, args_t<Ts, TEvent>, typename TSM_::logger_t>::execute(aux::get_by_id<Ns>(&g), event, sm, deps,
				        subs) &&
				        ...);
				#else
				auto result = true;
				(void)aux::swallow{ 0, (result = result && call<TEvent, args_t<Ts, TEvent>, typename TSM_::logger_t>::execute(
	...
	class not_ : operator_base {
		public:
			...
			template <class TEvent, class TSM_, class TDeps, class TSubs>
			auto operator()(const TEvent& event, TSM_& sm, TDeps& deps, TSubs& subs) {
				return !call<TEvent, args_t<T, TEvent>, typename TSM_::logger_t>::execute(g, event, sm, deps, subs);
			}
...
template < class T1, class T2, __BOOST_SML_REQUIRES(concepts::callable<void, T1>::value&&
...
namespace front {
	namespace actions {
		struct defer : action_base {
			template <class TEvent, class TSM_, class TDeps, class TSubs>
			void operator()(const TEvent& event, TSM_& sm, TDeps&, TSubs&) {
...
namespace concepts {
	...
	template <class T>
	auto transitional_impl(T&&
	                       t)->aux::always<typename T::dst_state, typename T::src_state, typename T::event, typename T::deps,
		...
		struct process {
				...
				class process_impl : public action_base {
					public:
						...
						template <class T, class TSM_, class TDeps, class TSubs>
						void operator()(const T&, TSM_&, TDeps&, TSubs& subs) {
